...
$WIT_HOME/bin/imagetool.sh cache addInstaller --path
$HOME/fmw_12.2.1.4.0_wls_lite_generic.jar --type wls --version 12.2.1.4.0 --force
$WIT_HOME/bin/imagetool.sh cache addInstaller --path $HOME/weblogicdeploy.zip --type wdt --version latest --force
$WIT_HOME/bin/imagetool.sh create --tag fra.ocir.io/sefinland/mrinne/vtest:$tag --version 12.2.1.4.0 --jdkVersion 8u281 --wdtModel ./wdt-model.yaml --wdtArchive ./wdt-archive.zip --wdtVariables ./vz_variable.properties --resourceTemplates ./application.yaml --wdtModelOnly
docker login -u $DOCKER_USER -p $DOCKER_PASSWORD fra.ocir.io
docker push fra.ocir.io/sefinland/mrinne/vtest:$tag
cd $HOME
